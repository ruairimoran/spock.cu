...
- All development will take place in branches out of `develop`
...
- When you create a new PR, make sure that the base branch is `develop` (not `main`), unless you want to create a new release (see below)
...
- No PR will be merged to `develop` unless it has been tested (evidence should be provided)
- The branch `develop` will be merged into `main` before making a new release (`CHANGELOG.md` must be updated when doing so)
...
In that case, you need to merge the base branch (usually `develop`) into the branch of the PR<sup>1</sup> and **resolve the conflicts locally** before you push. On [GitHub's documentation](https://docs.github.com/en/pull-requests/collaborating-with-pull-requests/addressing-merge-conflicts/about-merge-conflicts) we read 
...
<details>
  ...
  To merge `develop` into a branch, do 
