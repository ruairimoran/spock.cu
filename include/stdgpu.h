...
void gpuMatMul(Context& context, size_t m, size_t k, size_t n,
    ...
    const T alpha = 1.0;
    const T beta = 0.0;
    size_t lda, ldb, ldc = 0;
    if (not transA && not transB) { lda = k; ldb = n; ldc = n; }
    if (transA && not transB) { lda = m; ldb = n; ldc = n; }
    if (not transA && transB) { lda = k; ldb = k; ldc = n; }
    if (transA && transB) { lda = m; ldb = k; ldc = n; }
