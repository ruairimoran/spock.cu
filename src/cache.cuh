...
void Cache::offline_projection_setup() {
    ...
    m_d_choleskyUp.allocateOnDevice(m_numInputs * m_numInputs * m_tree.numNonleafNodes());
    ...
    for (size_t stage = m_tree.numStages() - 2; stage >= 0; stage--) {
        ...
        for (size_t node = nodeFrom; node <= nodeTo; node++) {
            ...
            DeviceVector<real_t> d_sumR(std::vector(m_numInputs * m_numInputs, 0.0));
            DeviceVector<real_t> d_sumK(std::vector(m_numInputs * m_numStates, 0.0));
            for (size_t child = chFrom; child <= chTo; child++) {
                size_t sizeP = m_numStates;
                ...
                DeviceVector<real_t> d_matB(m_data.inputDynamics(),
                                            getIdxMat(child, 0, 0, sizeP, m_numInputs),
                                            getIdxMat(child, sizeP, m_numInputs, sizeP, m_numInputs));
                ...
                gpuMatMul(m_data.context(), m_numInputs, sizeP, sizeP, d_matB, d_matP, d_matBP, true);
                DeviceVector<real_t> d_matBPB(m_numInputs * m_numInputs);
                gpuMatMul(m_data.context(), m_numInputs, sizeP, m_numInputs, d_matBP, d_matB, d_matBPB);
                DeviceVector<real_t> d_matBPA(m_numInputs * m_numStates);
                gpuMatMul(m_data.context(), m_numInputs, sizeP, m_numStates, d_matBP, d_matA, d_matBPA);
                gpuMatAdd(m_data.context(), m_numInputs, m_numInputs, d_matBPB, d_sumR, d_sumR);
                gpuMatAdd(m_data.context(), m_numInputs, m_numStates, d_matBPA, d_sumK, d_sumK);
            }
//            DeviceVector<real_t> d_idInput(m_numInputs * m_numInputs);
//            d_setMatToId<<<m_numInputs, THREADS_PER_BLOCK>>>(d_idInput.get(), 0, m_numInputs);
//            gpuMatAdd(m_data.context(), m_numInputs, m_numInputs, d_idInput, d_sumR, d_sumR);
