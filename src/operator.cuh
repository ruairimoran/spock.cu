...
void LinearOperator<T>::op(DTensor<T> &u, DTensor<T> &x, DTensor<T> &y, DTensor<T> &t, DTensor<T> &s,
    ...
    /* III */
    m_data.nonleafConstraint()->op(iii, x, m_numNonleafNodesMinus1, m_data.numStates(), u, m_data.numInputs());
    /* IV:1 */
    m_tree.memCpyAnc2Node(*m_d_xNonleafWorkspace, x, 1, m_numNodesMinus1, m_data.numStates(), 0, 0);
    m_d_xNonleafWorkspace->addAB(m_data.sqrtStateWeight(), *m_d_xNonleafWorkspace);
    /* IV:2 */
    m_tree.memCpyAnc2Node(*m_d_uNonleafWorkspace, u, 1, m_numNodesMinus1, m_data.numInputs(), 0, 0);
    ...
    /* IV (organise IV:1-4) */
    /* :1 */
    memCpyNode2Node(iv, *m_d_xNonleafWorkspace, 1, m_numNodesMinus1, m_data.numStates());
    /* :2 */
    memCpyNode2Node(iv, *m_d_uNonleafWorkspace, 1, m_numNodesMinus1, m_data.numInputs(), m_data.numStates());
    /* :3 */
    memCpyNode2Node(iv, t, 1, m_numNodesMinus1, 1, m_data.numStatesAndInputs());
    /* :4 */
    memCpyNode2Node(iv, t, 1, m_numNodesMinus1, 1, m_data.numStatesAndInputs() + 1);
    /* V */
    DTensor<T> xLeaf(x, m_matAxis, m_tree.numNonleafNodes(), m_numNodesMinus1);
    m_data.leafConstraint()->op(v, xLeaf, m_numLeafNodesMinus1, m_data.numStates());
    /* VI:1 */
    ...
    /* VI (organise VI:1-3) */
    /* :1 */
    memCpyNode2Node(vi, *m_d_xLeafWorkspace, 0, m_numLeafNodesMinus1, m_data.numStates());
    /* :2 */
    m_tree.memCpyLeaf2Zero(vi, s, 1, m_data.numStates(), 0);
    /* :3 */
    m_tree.memCpyLeaf2Zero(vi, s, 1, m_data.numStates() + 1, 0);
}
...
void LinearOperator<T>::adj(DTensor<T> &u, DTensor<T> &x, DTensor<T> &y, DTensor<T> &t, DTensor<T> &s,
    ...
    m_data.nonleafConstraint()->adj(iii, x, m_numNonleafNodesMinus1, m_data.numStates(), u, m_data.numInputs());
    /* x (nonleaf) and u:Weights */
    /* -> Compute `Qiv1` at every nonroot node */
    memCpyNode2Node(*m_d_xNonleafWorkspace, iv, 1, m_numNodesMinus1, m_data.numStates());
    m_d_xNonleafWorkspace->addAB(m_data.sqrtStateWeight(), *m_d_xNonleafWorkspace);
    /* -> Compute `Riv2` at every nonroot node */
    memCpyNode2Node(*m_d_uNonleafWorkspace, iv, 1, m_numNodesMinus1, m_data.numInputs(), 0,
                           m_data.numStates());
    m_d_uNonleafWorkspace->addAB(m_data.sqrtInputWeight(), *m_d_uNonleafWorkspace);
    ...
    /* t */
    memCpyNode2Node(t, iv, 1, m_numNodesMinus1, 1, 0, m_data.numStatesAndInputs());
    memCpyNode2Node(*m_d_scalarWorkspace, iv, 1, m_numNodesMinus1, 1, 0, m_data.numStatesAndInputs() + 1);
    t += *m_d_scalarWorkspace;
    t *= 0.5;
    /* x (leaf):Gamma */
    DTensor<T> xLeaf(x, m_matAxis, m_tree.numNonleafNodes(), m_numNodesMinus1);
    m_data.leafConstraint()->adj(v, xLeaf, m_numLeafNodesMinus1, m_data.numStates());
    /* x (leaf) */
    memCpyNode2Node(*m_d_xLeafWorkspace, vi, 0, m_numLeafNodesMinus1, m_data.numStates());
    xLeaf.addAB(m_data.sqrtStateWeightLeaf(), *m_d_xLeafWorkspace, 1., 1.);
    /* s (leaf) */
    m_tree.memCpyZero2Leaf(s, vi, 1, 0, m_data.numStates());
    m_tree.memCpyZero2Leaf(*m_d_scalarWorkspace, vi, 1, 0, m_data.numStates() + 1);
