...
void memCpy(DTensor<T> *dst, DTensor<T> *src,
    ...
    if (mode == leaf2ZeroLeaf) {
        ///< Must transfer all leaf nodes! So `nodeFrom` == numNonleafNodes.
 ...
 */
    ...
    void LinearOperator<T>::op(DTensor<T> &u, DTensor<T> &x, DTensor<T> &y, DTensor<T> &t, DTensor<T> &s,
        ...
        /* IV:1 */
        memCpy(m_d_xNonleafWorkspace.get(), &x, 1, numNodesMinusOne, m_data.numStates(), 0, 0,
               anc2Node, &m_tree.d_ancestors());
        m_d_xNonleafWorkspace->addAB(m_data.sqrtStateWeight(), *m_d_xNonleafWorkspace);
        /* IV:2 */
        memCpy(m_d_uNonleafWorkspace.get(), &u, 1, numNodesMinusOne, m_data.numInputs(), 0, 0,
               anc2Node, &m_tree.d_ancestors());
        ...
        /* VI (organise VI:1-3) */
        /* :1 */
        memCpy(&vi, m_d_xLeafWorkspace.get(), 0, m_tree.numLeafNodes()-1, m_data.numStates());
        /* :2 */
        memCpy(&vi, &s, m_tree.numNonleafNodes(), numNodesMinusOne, 1, m_data.numStates(), 0, leaf2ZeroLeaf);
        /* :3 */
        memCpy(&vi, &s, m_tree.numNonleafNodes(), numNodesMinusOne, 1, m_data.numStates() + 1, 0, leaf2ZeroLeaf);
    }
