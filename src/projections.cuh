...
public:
    explicit SocProjection(DTensor<T> &d_tensor) : Projection<T>(d_tensor) {
        ...
        m_t_ws = std::make_unique<DTensor<T>>(this->m_numCols);
        m_squaredStuff_ws = std::make_unique<DTensor<T>>(this->m_numCols * (this->m_numRows - 1));
        ...
        size_t blocksDimX = DIM2BLOCKS(this->m_numRows);
    ...
    void project(DTensor<T> &d_tensor) {
        ...
        k_projectionMultiSoc_s1<<<m_gridDims, m_threadsPerBlock>>>(d_tensor.raw(), this->m_numCols, this->m_numRows,
                                                                   m_t_ws->raw(), m_squaredStuff_ws->raw(),
                                                                   m_norms->raw(), m_i2->raw(), m_i3->raw(),
                                                                   m_scalingParams->raw());
        k_projectionMultiSoc_s2_i2<<<m_gridDims, m_threadsPerBlock>>>(d_tensor.raw(), this->m_numCols, this->m_numRows,
                                                                      m_i2->raw());
        k_projectionMultiSoc_s2_i3<<<m_gridDims, m_threadsPerBlock>>>(d_tensor.raw(), this->m_numCols, this->m_numRows,
                                                                      m_norms->raw(), m_i2->raw(), m_i3->raw(),
                                                                      m_scalingParams->raw());
    }
};
